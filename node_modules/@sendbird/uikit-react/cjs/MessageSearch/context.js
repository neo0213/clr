...
function reducer(state, action) {
    switch (action.type) {
        case SET_CURRENT_CHANNEL: {
            ...
            return _tslib.__assign(_tslib.__assign({}, state), { currentChannel: currentChannel, initialized: true });
        }
        case CHANNEL_INVALID: {
            return _tslib.__assign(_tslib.__assign({}, state), { currentChannel: null, initialized: false });
        }
        case GET_SEARCHED_MESSAGES: {
            ...
            if (createdQuery
                ...
                return _tslib.__assign(_tslib.__assign({}, state), { loading: false, isInvalid: false, allMessages: _tslib.__spreadArray([], messages, true), hasMoreResult: (state === null || state === void 0 ? void 0 : state.currentMessageSearchQuery).hasNext });
            }
        ...
        case START_MESSAGE_SEARCH: {
            return _tslib.__assign(_tslib.__assign({}, state), { isInvalid: false, loading: false });
        }
        case START_GETTING_SEARCHED_MESSAGES: {
            var currentMessageSearchQuery = action.payload;
            return _tslib.__assign(_tslib.__assign({}, state), { loading: true, currentMessageSearchQuery: currentMessageSearchQuery });
        }
        case GET_NEXT_SEARCHED_MESSAGES: {
            var messages = action.payload;
            return _tslib.__assign(_tslib.__assign({}, state), { allMessages: _tslib.__spreadArray(_tslib.__spreadArray([], state.allMessages, true), messages, true), hasMoreResult: (state === null || state === void 0 ? void 0 : state.currentMessageSearchQuery).hasNext });
        }
...
function useSearchStringEffect(_a, _b) {
    ...
    var _c = React.useState(''), requestString = _c[0], setRequestString = _c[1];
    var _d = React.useState(null), debouncingTimer = _d[0], setDebouncingTimer = _d[1];
    React.useEffect(function () {
...
var MessageSearchContext = React.createContext(undefined);
var MessageSearchProvider = function (props) {
    ...
    // hook variables
    var _f = React.useState(0), retryCount = _f[0], setRetryCount = _f[1]; // this is a trigger flag for activating useGetSearchMessages
    var _g = React.useState(0), selectedMessageId = _g[0], setSelectedMessageId = _g[1];
    var _h = React.useReducer(reducer, initialState), messageSearchStore = _h[0], messageSearchDispatcher = _h[1];
    ...
    return (React.createElement(MessageSearchContext.Provider, { value: {
...
var useMessageSearchContext = function () { return React.useContext(MessageSearchContext); };
