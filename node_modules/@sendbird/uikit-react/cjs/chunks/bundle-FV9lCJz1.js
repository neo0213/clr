...
var UserProfileContext = require('./bundle-gM8i5lGF.js');
    ...
    if (channelListQuery.hasNext) {
        channelListQuery
            ...
            channelListDispatcher({
                ...
                payload: { channelList: sortedChannelList, disableAutoSelect: disableAutoSelect },
            });
...
var pubSubHandler = function (pubSub, channelListDispatcher) {
    ...
    subscriber.set(pubSub_topics.pubSubTopics.SEND_MESSAGE_START, pubSub.subscribe(pubSub_topics.pubSubTopics.SEND_MESSAGE_START, function (_a) {
...
function channelListReducer(state, action) {
    return (index_module.K(action)
        .with({ type: INIT_CHANNELS_START }, function () { return (_tslib.__assign(_tslib.__assign({}, state), { loading: true })); })
        .with({ type: RESET_CHANNEL_LIST }, function () { return initialState; })
        ...
        return _tslib.__assign(_tslib.__assign({}, state), { initialized: true, loading: false, allChannels: channelList, disableAutoSelect: disableAutoSelect, currentChannel: !disableAutoSelect && channelList && channelList.length && channelList.length > 0 ? channelList[0] : null });
    })
        ...
        .with({ type: index_module.S.union(LEAVE_CHANNEL_SUCCESS, ON_CHANNEL_DELETED) }, function (action) {
        var _a;
        var channelUrl = action.payload;
        return _tslib.__assign(_tslib.__assign({}, state), { currentChannel: channelUrl === ((_a = state.currentChannel) === null || _a === void 0 ? void 0 : _a.url) ? state.allChannels[0] : state.currentChannel, allChannels: state.allChannels.filter(function (_a) {
                var url = _a.url;
        ...
        .with({
        type: index_module.S.union(ON_USER_JOINED, ON_CHANNEL_CHANGED, ON_READ_RECEIPT_UPDATED, ON_DELIVERY_RECEIPT_UPDATED),
    }, function (action) {
        var _a, _b;
        var channel = action.payload;
        ...
        return _tslib.__assign(_tslib.__assign({}, state), { allChannels: _tslib.__spreadArray([action.payload], state.allChannels.filter(function (channel) { return (channel === null || channel === void 0 ? void 0 : channel.url) !== action.payload.url; }), true) });
    })
        .with({ type: CHANNEL_LIST_PARAMS_UPDATED }, function (action) {
        return _tslib.__assign(_tslib.__assign({}, state), { currentUserId: action.payload.currentUserId, channelListQuery: action.payload.channelListQuery });
    })
...
//# sourceMappingURL=bundle-FV9lCJz1.js.map
